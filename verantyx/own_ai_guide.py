"""The build-your-own-AI guide — how data placement works and how to adjust it.

Rendered from code so the guide, the tools it names, and the file formats it
shows cannot drift apart; regenerated into docs/ the same way the settings
guide is. Bilingual in one document, because two documents is how one goes
stale.
"""
...
def render() -> str:
    st = ja_grammar.status()
    return f"""# 自分の AI を作る — データ配置ガイド / Build your own AI — data placement
...
ストップワード {st['stopwords']} 語、対義対 {st['pairs']} 対、
別名 {st['aliases']}、述語形 {st['predicates']}(lang_data/ja_grammar.json)。
