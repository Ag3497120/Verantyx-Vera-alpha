...
M = [
    ...
    ("M2", "max(a, b)", "max(b, a)"),
    ("M3", "min(a, add(a, b))", "a"),
    ("M4", "max(a, add(a, b))", "add(a, b)"),
    ("M5", "add(min(a, b), max(a, b))", "add(a, b)"),
    ("M6", "min(a, a)", "a"),
]
MF = [
    ("MF1", "min(a, b)", "a"),
...
_T = ("rcases Nat.le_total a b with h | h <;> "
      "simp [Nat.min_def, Nat.max_def, h] <;> omega")
_T2 = ("rcases Nat.le_total a (a+b) with h | h <;> "
       "simp [Nat.min_def, Nat.max_def, h] <;> omega")
LEAN_M = {
 "M1": "theorem t (a b : Nat) : Nat.min a b = Nat.min b a := by " + _T,
 "M2": "theorem t (a b : Nat) : Nat.max a b = Nat.max b a := by " + _T,
 "M3": "theorem t (a b : Nat) : Nat.min a (a+b) = a := by " + _T2,
 "M4": "theorem t (a b : Nat) : Nat.max a (a+b) = a+b := by " + _T2,
 "M5": "theorem t (a b : Nat) : Nat.min a b + Nat.max a b = a + b := by " + _T,
 "M6": "theorem t (a : Nat) : Nat.min a a = a := by simp",
}
LEDGER = Path(__file__).with_name("proof_ledger14.json")
GROUPS = [("confirm", CONFIRM), ("regress", REGRESS), ("battery", BATTERY),
          ("poly", POLY), ("lists", LISTS), ("false", FALSE),
          ("ineq", Q), ("ineq_false", QF), ("minmax", M),
          ("minmax_false", MF)]
...
def main():
    ...
    # 断面合意(PREREG14): 全目標で断面間の 証明↔反駁 矛盾を検査
    sec_conflicts = []
    sec_stats = []
    for gname, goals in GROUPS:
        for (name, l, r), g1 in zip(goals, res[gname]):
            p = Prover(mathlib_context=ml)
            s = p.prove_sections(parse_term(l), parse_term(r))
            sec_stats.append({"goal": name, **s})
            if s["conflict"]:
                sec_conflicts.append({"goal": name, "sections": s["sections"]})
    res["section_conflicts"] = sec_conflicts
    res["section_agree_summary"] = {
        "goals": len(sec_stats),
        "multi_section_proved": sum(1 for s in sec_stats
                                    if s["agree_proved"] >= 2)}
    # 表面伝導の対照(報告のみ)
    import prover as PP
    PP.SURFACE_CONDUCTION = False
    off = {"proved": 0, "nodes": 0}
    for gname, goals in GROUPS:
        for name, l, r in goals:
            p = Prover(mathlib_context=ml)
            ok, how = p.prove(parse_term(l), parse_term(r))
            off["proved"] += bool(ok)
            off["nodes"] += p.stats["nodes"]
    PP.SURFACE_CONDUCTION = True
    on_nodes = sum(g["nodes"] for gname, _ in GROUPS for g in res[gname])
    on_proved = sum(1 for gname, _ in GROUPS for g in res[gname]
                    if g["verdict"] == "proved")
    res["conduction_ablation"] = {"on": {"proved": on_proved,
                                         "nodes": on_nodes},
                                  "off": off}
    ledger.save()
    ...
    Path(__file__).with_name("results_confirm14.json").write_text(
