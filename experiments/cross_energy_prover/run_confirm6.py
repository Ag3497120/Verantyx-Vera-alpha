# -*- coding: utf-8 -*-
"""確認測定6 — PREREG6.md が事前登録。持ち越しの向きを LPO が決める。"""
import json, sys, time
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
from prover import (DEFS, Prover, _defs_are_lpo_decreasing,
                    generalise_str, orient_by_lpo, parse_term)
...
LEDGER = Path(__file__).with_name("proof_ledger6.json")
...
def main():
    ...
    while True:
        ...
        for name, l, r in pending:
            ...
            if ok:
                ...
                ol, orr, kind = orient_by_lpo(tl, tr)
                rules = rules + [(name, generalise_str(ol),
                                  generalise_str(orr))]
                if kind == "undirected":
    ...
    res = {"defs_lpo_violations": _defs_are_lpo_decreasing(),
           "rounds": rounds,
    ...
    Path(__file__).with_name("results_confirm6.json").write_text(
