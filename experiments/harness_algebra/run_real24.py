...
PAD = ("これは無関係な前置きである。当社の沿革は長く、創業者は登山を好み、"
       ...
       "食堂の献立は季節で変わる。") * 6   # ~約500字の邪魔

def _arith(q, ans):
    return {"prompt": f"{q} 数字だけで答えよ。", "expect": ans,
            "judge": "digits", "kind": "arith"}

def _json(keys, seed, expect):
    return {"prompt": PAD[:300] + f" 次の指示だけに従え: キー {keys} を持つJSONを返せ。値は {seed}。JSONのみ。",
            "expect": expect, "judge": "json:" + ",".join(sorted(expect)),
            "kind": "json"}

def _extract(fact, q, ans):
    return {"prompt": PAD[:250] + fact + PAD[250:500] + f" 質問: {q} 答えだけを書け。",
            "expect": ans, "judge": "short", "kind": "extract"}

TASKS = [
    _arith("37+58は?", "95"), _arith("84-29は?", "55"),
    _arith("17*6は?", "102"), _arith("73+69は?", "142"),
    _arith("91-47は?", "44"), _arith("23*7は?", "161"),
    _json("name と id", "name=kikai, id=7", {"name": "kikai", "id": 7}),
    _json("x と y", "x=3, y=9", {"x": 3, "y": 9}),
    _json("code と ok", "code=A1, ok=true", {"code": "A1", "ok": True}),
    _extract("ところで倉庫の棚番号は B417 である。", "倉庫の棚番号は?", "B417"),
    _extract("なお当直の内線は 3062 である。", "当直の内線は?", "3062"),
    _extract("ちなみに保管庫の暗証は棚卸日と同じ 1130 である。", "保管庫の暗証は?", "1130"),
    # PREREG3: 追加12課題(採択線をノイズ床の上へ — 変数は課題数のみ)
    _arith("56+87は?", "143"), _arith("93-38は?", "55"),
    _arith("14*9は?", "126"), _arith("68+55は?", "123"),
    _arith("82-46は?", "36"), _arith("19*8は?", "152"),
    _json("user と score", "user=hana, score=42", {"user": "hana", "score": 42}),
    _json("a と b と c", "a=1, b=2, c=3", {"a": 1, "b": 2, "c": 3}),
    _json("tag と count", "tag=z9, count=5", {"tag": "z9", "count": 5}),
    _extract("第2会議室の内線は 4408 である。", "第2会議室の内線は?", "4408"),
    _extract("検収日は毎月 25日 と定められている。", "検収日は毎月何日?", "25日"),
    _extract("更衣室のロッカー番号は C205 が予備である。", "予備のロッカー番号は?", "C205"),
]
...
def main():
    ...
    split = []
    for v in VARIANTS[1:]:
        va = next(f["verdict1"] for f in res["models"]["small"]["facts"]
                  if f["fact"] == f"{v}_helps")
        vb = next(f["verdict1"] for f in res["models"]["mid"]["facts"]
                  if f["fact"] == f"{v}_helps")
        if va != vb:
            split.append({"fact": f"{v}_helps", "small": va, "mid": vb})
    ...
    Path(__file__).with_name("results_real24.json").write_text(
